...
import com.itssinghankit.stockex.util.dateTimeToTime
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.flow
import javax.inject.Inject

class GetDayGraphDataUseCase @Inject constructor(
    private val repositoryInterface: RepositoryInterface
) {
    @RequiresApi(Build.VERSION_CODES.O)
    suspend operator fun invoke(symbol: String): Flow<ApiResult<List<ChartModel>, DataError.Network>> =
        flow {
            val response = repositoryInterface.getDayGraphData(symbol)
                ...
                when (result) {
                    ...
                    is ApiResult.Success -> {
                        val data = result.data.map {
                            val outputLabel =
                                "$${String.format("%.2f", it.value)} | ${dateTimeToTime(it.label)}"
                            ChartModel(
                                label = outputLabel,
                                value = it.value
                            )
